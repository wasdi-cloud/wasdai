...
async def run_processor(
    ...
    sWorkspaceId: is the unique id of the workspace in which the processor will run.    
    sJsonParams: is the PLAIN JSON string containing the parameters for the processor. The agent must provide raw, unencoded JSON here.
    ...
    if sJsonParams is None:
        raise ValueError("Missing json payload")

    # Perform the URL encoding locally
    sEncodedJson = urllib.parse.quote(sJsonParams)
    ...
    async with httpx.AsyncClient() as oClient:
        oResponse = await oClient.post(
            ...
            params=aoParams,
            content=sEncodedJson, # Pass the Python-encoded string here
            headers={"x-session-token": sSessionToken},
        )
        oResponse.raise_for_status()
